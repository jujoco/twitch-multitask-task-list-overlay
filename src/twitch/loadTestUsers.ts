import type TwitchChat from './TwitchChat.js';

export function loadTestUsers(twitchClient: TwitchChat): void {
	...
	for (let i = 1; i <= 5; i++) {
		...
		for (let j = 0; j < maxTasksPerUser; j++) {
			const data = {
				user: userName,
				command: 'task',
				message: `test task description ${j === 2 ? 'longer text example' : ''}`,
				flags: { broadcaster: true, mod: false },
				extra: {
					userColor: userColor,
					messageId: `${generateTimeStamp()}`,
