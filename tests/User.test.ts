...
describe('User', () => {
	...
	describe('validateUsername', () => {
		...
		test('should throw an Error if username is not a string', () => {
			...
			expect(() => user.validateUsername(123)).toThrow('Username must be of type string');
			// @ts-expect-error testing invalid runtime input
			expect(() => user.validateUsername(true)).toThrow('Username must be of type string');
			// @ts-expect-error testing invalid runtime input
			expect(() => user.validateUsername([])).toThrow('Username must be of type string');
		});
	});
