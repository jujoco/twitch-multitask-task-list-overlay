...
describe('UserList', () => {
	...
	describe('createUser', () => {
		...
		test('should throw an error if user already exists', () => {
			userList.createUser('user1', { userColor: '#ff0000' });
			expect(() => userList.createUser('user1', { userColor: '#ff0000' })).toThrow(
	...
	describe('addUserTasks', () => {
		test('should be able to add a single string description', () => {
			...
			expect(task1.description).toEqual('A single task containing , comma in, description');
		});
		...
		test('should be able to add multiple tasks if given an array of descriptions', () => {
			...
			expect(task2.description).toEqual('Task 2 containing , comma in, description');
		});

		test('should throw an error if user does not exist', () => {
			expect(() => userList.addUserTasks('nonExistentUser', 'Task 1')).toThrow(
	...
	describe('editUserTask', () => {
		...
		test('should throw an error if task does not exist', () => {
			userList.createUser('user1', { userColor: '#ff0000' });
			userList.addUserTasks('user1', 'Task 1');
			expect(() => userList.editUserTask('user1', 1, 'Updated Task 1')).toThrow('Task 1 not found');
		});

		test('should throw an error if user does not exist', () => {
			expect(() => userList.editUserTask('nonExistentUser', 0, 'Updated Task 1')).toThrow(
	...
	describe('completeUserTask', () => {
		...
		test('should be able to mark multiple tasks as complete if given an array of indices', () => {
			userList.createUser('user1', { userColor: '#ff0000' });
			userList.addUserTasks('user1', ['Task 1', 'Task 2', 'Task 3', 'Task 4']);
		...
		test('should throw an error if user does not exist', () => {
			expect(() => userList.completeUserTasks('nonExistentUser', 0)).toThrow(
	...
	describe('setFocusedTask', () => {
		...
		test('should throw an error if user does not exist', () => {
			expect(() => userList.focusUserTask('nonExistentUser', 0)).toThrow(
				'User nonExistentUser not found',
			);
		});
	});
	...
	describe('deleteUserTask', () => {
		...
		test('should throw an error if user does not exist', () => {
			expect(() => userList.deleteUserTasks('nonExistentUser', [0])).toThrow(
				'User nonExistentUser not found',
			);
		});
	});
	...
	describe('clearDoneTasks', () => {
		test('should clear all done tasks for the user', () => {
			const _user1 = userList.createUser('user1', {
				userColor: '#ff0000',
			});
			userList.addUserTasks('user1', ['user1-task 1', 'user1-task 2', 'user1-task 3']);
			userList.completeUserTasks('user1', 0);

			const _user2 = userList.createUser('user2', {
				userColor: '#ff0000',
			});
	...
	describe('deleteUser', () => {
		...
		test('should throw an error if user does not exist', () => {
			expect(() => userList.deleteUser('nonExistentUser')).toThrow('User not found');
		});
	});
});
