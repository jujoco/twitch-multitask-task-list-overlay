...
describe('Task', () => {
	...
	describe('validateDescription', () => {
		...
		it('should throw an error if the description is invalid', () => {
			expect(() => task.validateDescription('')).toThrow('Task description invalid');
		});
		...
		it('should throw Error if description is not a string', () => {
			...
			// @ts-expect-error testing invalid runtime input
			expect(() => task.validateDescription([])).toThrow('Task description must be of type string');
			// @ts-expect-error testing invalid runtime input
			expect(() => task.validateDescription({})).toThrow('Task description must be of type string');
		});
	});
	...
	describe('setFocusStatus', () => {
		...
		it('should return Error if status is not a boolean', () => {
			// @ts-expect-error testing invalid runtime input
			expect(() => task.setFocusStatus('true')).toThrow('Focus status must be of type boolean');
		});
	});
});
